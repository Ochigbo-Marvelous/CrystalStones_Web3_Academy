  ...
  useEffect(() => {
    ...
    timer = window.setInterval(async () => {
      setOrder((current) => {
        ...
        fetch(`${API}/api/payments/${current.id}/sync`, { method: "POST", headers })
          .then((res) => res.json())
          .then((data) => {
            if (data?.data) {
              setOrder(data.data);
      ...
      <div className="su-card" style={{ width: "min(100%, 480px)" }}>
        ...
        {order ? (
          <>
            ...
            <p>${order.amount_usd} ≈ {order.amount_usdt || order.amount_crystal} USDT</p>
            ...
            <p className="su-note">Status: {order.status}. This page checks payment every 15 seconds.</p>
          </>
        ) : null}
        <p className="su-foot"><Link to="/dashboard">Back to dashboard</Link></p>
      </div>
    </div>
  );
}